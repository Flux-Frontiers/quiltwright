...
- **`geometry="toe-in"` on `render_pov_views()`, and `toe_in_cameras()`
  ...
  plain symmetric frustum. This is the projection the module docstring
  tells you not to use, and it is here because LitiHolo's own capture tool
  uses it: its web renderer orbits a Sketchfab camera around a pinned aim
  point and screenshots 23 stops, with no frustum shear anywhere in it. A
  hogel slicer is not a lens sheet, and if it has only ever been fed toe-in
  view sets then an off-axis set is an unlabelled change of input rather
  than a fix. Off-axis remains the default, `render_pov_quilt()` does not
  take the option at all, and the emitted wrapper comment names which
  geometry produced it.
- **`LITIHOLO_TOOL_SWEEP`,** the sweep LitiHolo's tool emits at its factory
  defaults, kept separate from `LITIHOLO_SWEEP`, which is the specification
  sheet. They disagree: the tool steps 2.5 degrees per move and captures 23
  stops from -27.5 to +27.5, so its cone is **55 degrees, not 45**, and its
  default capture is 400x400 square rather than the 1600x2000 this project
  guessed. Neither number is a specification -- both are fields in the
  tool's UI -- but they are what the machine has actually been fed.
- **`pack_litiholo_sweep()` and `litiholo_names()`,** the delivery step: a
  ...
  (`render-400x400-HPO-01.jpg` through `-23.jpg`, one-indexed, view 0
  leftmost), optionally with the `-HPO-captures.zip` its tool downloads.
  Renderer-agnostic, the same role `assemble_quilt()` plays for a panel, and
  it changes no geometry. Transparent frames are composited onto black
  rather than having their alpha discarded, which is the ground the tool's
  own transparent capture ends up with and the one background a hologram
  must not get wrong.
- **`view_angles()`,** the angle list both geometries sample. `view_offsets()`
  is now derived from it, so an off-axis and a toe-in sweep of the same spec
  provably sample the same angles and a depth budget's quoted sampling
  interval means the same thing either way.

### Documentation

- **[docs/lfd.md](docs/lfd.md) records what the vendor tool settles and what
  it does not.** Off-axis versus toe-in was the first of the four questions
  put to Liti Holographics in August and is now answered by example: the tool
  is toe-in, and the 2003 submission of one of these same scenes was toe-in
  too, and printed. What remains unknown is whether the slicer *requires*
  toe-in or merely tolerates it, and whether it resamples a denser source
  set. The tool's full-parallax mode -- 23x17 = 391 images over 55 by 40
  degrees -- is documented as a gap: quiltwright renders no vertical sweep,
  and `litiholo_names(mode="FP")` raises rather than name a grid it cannot
  fill.
