...
// museum_2026.pov -- the modern variant of museum_pg.pov (the 1999 cut),
// composed for holographic output rather than for 10x8 paper.
//
// Three changes from the original, and nothing else:
//   * MIRROR_FRAME true, so the oval mirror hangs where the Risedronate
//     picture did.  The mirror is the original scene's own object; this
//     only picks the branch that was already there.
//   * ASPECT 16/9 rather than 5/4.
//   * Both pedestals move inward to flank the alcove -- x 16 -> 34 and
//     80 -> 64, with their spotlights moved to match.
//
// museum_pg.pov itself is untouched, per the read-only rule in
// docs/pov-workflow.md.
//
#version 3.0
...
#declare MIRROR_FRAME = true    // oval mirror on wall -- 2026: this replaces
                                // the Risedronate picture that hung here, and
                                // the mirror is the original's own object, not
                                // a new one
...
#declare ASPECT=16/9 // 2026: the 16" landscape panel, not 10x8 paper
...
object {zdna_bell_jar_on_stand translate <64, .25, 37>}   // 2026: was 80
...
object {bdna_stand translate <22.5, .25, 37>}   // 2026: was 16; centred under lambdapic at x=25
// light it up
light_source {
   <19,40,20> color Gray85
   spotlight
   point_at <25,20,38>
   radius 15
   falloff 25
